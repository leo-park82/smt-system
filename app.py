            ...
            if IS_EDITOR:
                with st.container(border=True):
                    ...
                    def save_production():
                        ...
                        if cur_name:
                            rec = {"날짜":str(date), "구분":cat, "품목코드":cur_code, "제품명":cur_name, "수량":cur_qty, "입력시간":str(datetime.now()), "작성자":get_user_id(), "수정자":"", "수정시간":""}
        ...
        if not df.empty:
            ...
            if HAS_ALTAIR:
                ...
                with c1:
                    ...
                    c = alt.Chart(chart_data).mark_bar(color='#818cf8').encode(x=alt.X('날짜', axis=alt.Axis(format='%m-%d', labelAngle=0, title='날짜')), y=alt.Y('수량', axis=alt.Axis(labelAngle=0, titleAngle=0, title='수량')), tooltip=['날짜', '수량']).interactive()
            ...
            if IS_EDITOR:
                with st.container(border=True):
                    ...
                    if st.button("이력 저장", type="primary", use_container_width=True):
                        ...
                        rec = {"날짜": str(f_date), "설비ID": f_eq, "설비명": eq_name, "작업구분": f_type.split()[0], "작업내용": f_desc, "교체부품": parts_str, "비용": f_cost, "작업자": get_user_id(), "비가동시간": f_down, "입력시간": str(datetime.now()), "작성자": get_user_id()}
                        with st.spinner("저장 중..."):
