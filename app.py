...
USERS = {
    # [수정] 권한 설정 적용: admin(박종선), worker(김윤석, 김명숙)
    ...
    if not st.session_state.logged_in:
        try:
            ...
            if "session" in qp:
                ...
                # 뷰어 자동 로그인 처리 (세션 정보가 'guest'인 경우)
                elif saved_id == "guest":
                    st.session_state.logged_in = True
...
with st.sidebar:
    ...
    # 역할에 따른 뱃지 표시
    role_map = {"admin": "👑 관리자", "worker": "👷 작업자", "viewer": "👀 뷰어"}
    role_badge = role_map.get(u["role"], "👤 User")
...
# ------------------------------------------------------------------
# 5. 메인 기능 구현 (메뉴 이동 시 잔상 제거를 위한 컨테이너 격리)
# ------------------------------------------------------------------
            ...
            with t1:
                ...
                with c1:
                    ...
                    if st.session_state.user_info['role'] in ['admin', 'worker']:
                        ...
                        st.info("🔒 생산 실적 등록 권한이 없습니다. (조회 전용)")
                ...
                with c2:
                    ...
                    if not df.empty:
                        ...
                        if st.session_state.user_info['role'] == 'admin':
                            ...
                            # 관리자 아니면 그냥 표만 보여줌
                            st.dataframe(df.sort_values("입력시간", ascending=False).head(50), hide_index=True, use_container_width=True)
            ...
            with t1:
                ...
                with c1:
                    ...
                    if st.session_state.user_info['role'] in ['admin', 'worker']:
                        ...
                        st.info("🔒 정비 이력 등록 권한이 없습니다. (조회 전용)")
        ...
        try:
            ...
            with tab1:
                ...
                if len(lines) > 0:
                    ...
                    # [수정] 뷰어는 입력 비활성화 처리 (disabled)
                    is_disabled = st.session_state.user_info['role'] == 'viewer'
                        ...
                        for _, row in group.iterrows():
                            ...
                            with c2:
                                if not is_numeric and check_type == 'OX':
                                    ...
                                    val = st.radio("판정", ["OK", "NG"], key=widget_key, index=idx, horizontal=True, label_visibility="collapsed", disabled=is_disabled)
                            ...
                            if is_ng:
                                st.text_input("⚠️ 장비점검 (조치내역)", value=default_memo, key=memo_key, placeholder="NG 사유 및 조치내용 입력", disabled=is_disabled)
                    ...
                    signature_data = None
                    if HAS_CANVAS:
                        # 뷰어는 서명 불가 (key 변경해서 렌더링만)
                        canvas_result = st_canvas(
                            fill_color="rgba(255, 165, 0, 0.3)", stroke_width=2, stroke_color="#000000",
                            background_color="#ffffff", height=150, width=400, drawing_mode="freedraw" if not is_disabled else "transform",
                    ...
                    signer_name = c_s1.text_input("점검자 성명", value=st.session_state.user_info['name'], key=f"signer_{selected_line}", disabled=is_disabled)
            ...
            st.error(f"일일점검관리 로딩 중 오류 발생: {e}")

    elif menu == "⚙ 기준정보관리":
        # [수정] 뷰어 및 작업자는 접근 불가 처리
        if st.session_state.user_info['role'] == 'admin':
            try:
