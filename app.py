                        ...
                        for _, row in group.iterrows():
                            ...
                            with c2:
                                if row['check_type'] == 'OX':
                                    st.radio("판정", ["OK", "NG"], key=key_val, horizontal=True, label_visibility="collapsed", index=0 if prev.get('ox')=='OK' else 1 if prev.get('ox')=='NG' else 0)
                                else:
                                    st.number_input("수치", key=key_val, step=0.1, value=float(prev.get('val')) if prev.get('val') and prev.get('val').replace('.','',1).isdigit() else None)
