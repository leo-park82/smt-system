...
st.set_page_config(
    ...
    page_icon="🏭",
    layout="wide",
    # [수정] 태블릿/모바일에서 사이드바가 화면을 가리지 않도록 'auto'로 변경
...
def check_password():
    ...
    # 태블릿에서는 컬럼 비율을 조정하여 입력창이 너무 좁아지지 않게 함
    c1, c2, c3 = st.columns([1, 10, 1]) 
    with c2:
        # PC에서는 좁게, 모바일에서는 넓게 쓰기 위해 내부에 다시 컬럼 분할
        sc1, sc2, sc3 = st.columns([1, 1.2, 1])
        
        # 화면이 작으면 중앙 컬럼을 넓게 씀
        if st.sidebar.empty: # 모바일 감지 힌트 (완벽하진 않음)
...
def create_pdf_report(daily_df, report_date):
    ...
    # 리눅스/서버 환경 대비 폰트 경로 예외처리
    if not os.path.exists(font_path):
        font_path = "NanumGothic.ttf" # 로컬 폴더에 폰트가 있다면 우선 사용
    ...
    # 한글 깨짐 방지를 위해 타이틀 변경 고려 (폰트 없을 시)
        ...
        with c2:
            ...
            if not df.empty:
                ...
                if IS_EDITOR:
                    ...
                    # [수정] 관리자만 삭제(행 추가/제거) 가능, 일반 사용자는 수정만 가능
