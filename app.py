...
# [안전 장치] Matplotlib 백엔드 설정 (GUI 충돌 방지)
...
st.markdown("""
    ...
    /* 선택된 항목 스타일 (Streamlit DOM 구조에 따라 제한적 적용) */
            ...
            with t1:
                ...
                with c1:
                    if st.session_state.user_info['role'] in ['admin', 'worker']:
                        with st.container(border=True):
                            ...
                            # Session State 초기화 방어 코드
                            if "prod_qty" not in st.session_state: st.session_state.prod_qty = 100
                            if "code_in" not in st.session_state: st.session_state.code_in = ""
                ...
                with c2:
                    ...
                    if not df.empty:
                        ...
                        recent_df['입력시간_dt'] = pd.to_datetime(recent_df['입력시간_norm'], errors='coerce')
                        recent_df['입력시간_표시'] = recent_df['입력시간_dt'].dt.strftime('%Y-%m-%d %H:%M').fillna("-")
                        
                        recent_df = recent_df.sort_values(by=['날짜_dt', '구분', '입력시간_dt'], ascending=[False, True, False])
                        ...
                        if st.session_state.user_info['role'] == 'admin':
                            ...
                            st.dataframe(
                                ...
                                use_container_width=True,
                                column_config={
                                    "입력시간": st.column_config.DatetimeColumn("입력시간", format="YYYY-MM-DD HH:mm"),
                                    "수량": st.column_config.NumberColumn("수량", format="%d")
                                }
                            )
...
# [핵심] 리런 로직을 앱의 가장 마지막에 배치하여
# 모든 UI 렌더링 후 안전하게 리로드되도록 함
