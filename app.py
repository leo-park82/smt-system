...
if menu == "📊 대시보드":
    ...
    with tab1:
        ...
        if st.button("✅ 일괄 합격 처리 (빈 항목 OK 채우기)"):
            for _, row in df_master.iterrows():
                widget_key = f"check_val_{row['line']}_{row['equip_id']}_{row['item_name']}"
        ...
        with st.form("check_form"):
            # 라인별 그룹핑
            for line_name, line_group in df_master.groupby("line", sort=False):
                st.markdown(f"### 📍 {line_name}")
                
                # 설비별 그룹핑
                for equip_name, group in line_group.groupby("equip_name", sort=False):
            ...
            if st.form_submit_button("💾 점검 결과 전체 저장", type="primary", use_container_width=True):
                if signer_name:
                    rows_to_save = []
                    ng_list = []
    ...
    with tab2:
        ...
        c1.metric("진행률 (완료/전체)", f"{done_items} / {total_items}")
